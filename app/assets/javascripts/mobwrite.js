...
mobwrite.shareTextareaObj.prototype.getClientText = function() {
  ...
  var text = mobwrite.shareTextareaObj.normalizeLinebreaks_((this.element.value || this.element.innerHTML));
...
mobwrite.shareTextareaObj.shareHandler = function(node) {
  ...
  if (node && ('type' in node && (node.type == 'textarea' ||
      node.type == 'text' || node.type == 'password')) || ( 'contentEditable' in node && node.contentEditable == 'true')) {
    if (mobwrite.UA_webkit) {
